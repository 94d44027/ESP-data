...
- The TTA value: sum of TTB properties of all Asset tags in the path excluding the first (entry point) node

The response SHALL be ordered by TTA ascending. Both `from` and `to` parameters SHALL be validated per REQ-025. The hops parameter SHALL be validated as an integer in range 2–9; if omitted, default to 6.

Justification for MATCH syntax (per REQ-244): variable-length path traversal with loop detection (`ALL(n IN nodes(p) WHERE single(m IN nodes(p) WHERE m == n))`) and aggregation along path nodes has no practical nGQL/GO equivalent. The underlying query:
...
>Note: Note: The path IDs are generated by the APP layer (Go code) sequentially per response — they are ephemeral and not persisted. A different run with different mitigations may produce a different ordering, so P00001 may refer to a different path. Candidate for the future change.
