...
- After path calculation that included path-scoped recalculation (UI-REQ-112A)

**UI-REQ-112A: Stale Badge Update After Path-Scoped Recalculation**

 **Context:** UI-REQ-112 defines the stale badge and its refresh triggers. ALG-REQ-046 defines path-scoped TTB recalculation. When path-scoped recalculation successfully recalculates stale assets, the `stale_count` on SystemState SHALL be decremented accordingly.

**Requirement:** When `PathsHandler` (ALG-REQ-046 step 4) successfully recalculates one or more stale intermediate assets, the APP layer SHALL decrement `SystemState.stale_count` by the number of successfully recalculated assets, flooring at 0.

**Rationale:** Without this, the stale badge remains stale after path-scoped recalculation, misleading the user into thinking manual recalculation is still needed.

> **Note:** The VIS layer already calls `refreshSystemState()` after path-scoped recalculation (path-inspector.js after comment `// Show stale path warning if recalculation occurred (UI-REQ-113)`). This fix is APP-layer only — no JS changes needed.
...
| 1.13    | Mar 11, 2026 | §1.1 ALGO version updated (v1.5). UI-REQ-207 §5: TTA column format changed from integer to float (2 decimal places). UI-REQ-2091 added (TTB Calculation Parameters — Orientation Time, Switchover Time, Priority Tolerance controls in Path Inspector). Future features checklist updated (mitigation impact partially addressed). Appendix B updated. | AI + KSmirnov   |
| 1.14    | Mar 12, 2026 | Added UI-REQ-112A to clear the stale warning after the path calculation actually calculated the TTB for teh changed asset.                                                                                                                                                                                                                             | AI + KSmirnov   |
